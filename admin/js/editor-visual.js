(() => {
  ...
  async function addButton(){if(!selectedPage)return;const {data,error}=await db.from("pagina_elementos").insert({pagina_id:selectedPage.id,tipo:"botao",conteudo:"Novo botão",posicao_x:30,posicao_y:15,largura:40,altura:8,ordem:elements.length+1}).select().single();if(error){toast(error.message,"error");return}await loadElements();selectElement(data.id);setTimeout(()=>el.elementForm.scrollIntoView({behavior:"smooth",block:"start"}),80);toast("Botão criado. Escolha a aparência e o destino.")}
  async function saveElement(event){event.preventDefault();if(!selectedElement)return;const payload={conteudo:el.elementText.value.trim()||"Botão",estilos:{...(selectedElement.estilos||{}),aparencia:el.elementAppearance.value},posicao_x:clamp(el.x.value,0,98),posicao_y:clamp(el.y.value,0,98),largura:clamp(el.width.value,2,100),altura:clamp(el.height.value,2,100),destino_tipo:el.destinationType.value,destino_id:['pagina','categoria','produto'].includes(el.destinationType.value)?el.destinationId.value||null:null,destino_url:el.destinationType.value==="url"?el.destinationUrl.value.trim()||null:null};
