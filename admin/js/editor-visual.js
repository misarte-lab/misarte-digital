(() => {
  ...
  const el={loader:$("#pageLoader"),app:$("#editorApp"),back:$("#backLink"),catalog:$("#catalogName"),client:$("#clientName"),pages:$("#pagesList"),newPage:$("#newPageButton"),duplicate:$("#duplicatePageButton"),deletePage:$("#deletePageButton"),empty:$("#emptyCanvas"),shell:$("#canvasShell"),canvas:$("#pageCanvas"),pageName:$("#pageName"),pageType:$("#pageType"),background:$("#backgroundInput"),backgroundStatus:$("#backgroundStatus"),savePage:$("#savePageButton"),addButton:$("#addButtonElement"),preview:$("#previewButton"),elementForm:$("#elementProperties"),elementText:$("#elementText"),elementImage:$("#elementImage"),x:$("#elementX"),y:$("#elementY"),width:$("#elementWidth"),height:$("#elementHeight"),destinationType:$("#destinationType"),destinationField:$("#destinationSelectField"),destinationId:$("#destinationId"),destinationUrlField:$("#destinationUrlField"),destinationUrl:$("#destinationUrl"),deleteElement:$("#deleteElementButton"),toast:$("#toast")};
  let pages=[],elements=[],categories=[],products=[],selectedPage=null,selectedElement=null;
  ...
  el.pages.addEventListener("click",event=>{const button=event.target.closest("[data-page-id]");if(button)selectPage(button.dataset.pageId)});el.newPage.addEventListener("click",createPage);el.savePage.addEventListener("click",savePage);el.duplicate.addEventListener("click",duplicatePage);el.deletePage.addEventListener("click",deletePage);el.addButton.addEventListener("click",addButton);el.elementForm.addEventListener("submit",saveElement);el.deleteElement.addEventListener("click",deleteElement);el.destinationType.addEventListener("change",()=>updateDestinationOptions());el.preview.addEventListener("click",openPreview);
